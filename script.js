...
document.getElementById('generateBtn').addEventListener('click', () => {
    ...
    const seeded = document.getElementById('seededPlayers').value.split('\n').map(p => p.trim()).filter(p => p !== "");
    const unseeded = document.getElementById('unseededPlayers').value.split('\n').map(p => p.trim()).filter(p => p !== "");
    
    if (seeded.length !== nSeeds) return alert(`Expected ${nSeeds} seeds, found ${seeded.length}.`);
...
function toggleAdv(r, m, pNum) {
    ...
    if (match.winner === p) clearRes(r, m);
    else {
        match.winner = p;
        if (r < tournamentRounds.length - 1) {
            ...
            else if (m % 2 === 0) nextR[nextM].p1 = p; else nextR[nextM].p2 = p;
        }
    }
    renderTournament();
}
...
function clearRes(r, m) {
    ...
    if (r < tournamentRounds.length - 1) {
        const nextR = tournamentRounds[r+1], nextM = Math.floor(m / 2);
        if (r === tournamentRounds.length - 2) { if (nextR[0].p1 === prevW) nextR[0].p1 = "TBD"; }
        else {
            const slot = (m % 2 === 0) ? 'p1' : 'p2';
            if (nextR[nextM][slot] === prevW) { nextR[nextM][slot] = "TBD"; if (nextR[nextM].winner === prevW) clearRes(r + 1, nextM); }
        }
    }
    renderTournament();
}
    ...
    tournamentRounds.forEach((round, rIndex) => {
        ...
        round.forEach((match, mIndex) => {
            ...
            if (rIndex === tournamentRounds.length - 1) {
                content.innerHTML = `<div class="champion-box" onclick="clearRes(${rIndex}, ${mIndex})">${match.p1 === 'TBD' ? '???' : '🏆 ' + match.p1}</div>`;
            } else {
                content.innerHTML = `
            ...
            box.appendChild(content); col.appendChild(box);
        });
        container.appendChild(col);
    });
}

// 3. VERTICAL PORTRAIT PDF EXPORT
document.getElementById('downloadBtn').addEventListener('click', async function() {
    ...
    for (let p = 0; p < totalPages; p++) {
        ...
        columns.forEach((col, cIdx) => {
            ...
            if (isChampionCol) return; // Strictly exclude Champion from PDF
            ...
            matches.slice(start, end).forEach(m => {
                ...
                mClone.querySelectorAll('span').forEach(s => {
                    if (s.innerText.trim() === "TBD") s.className = "pdf-blank";
                });
...
function generateInvertedSeeding(seeded, unseeded, nSeeds, n) {
    ...
    while (seeds.length < n) {
        let next = [];
        for (let s of seeds) { next.push(s); next.push(seeds.length * 2 + 1 - s); }
        seeds = next;
    }
    const qSize = n / 4;
    const tennisOrder = [...seeds.slice(0, qSize), ...seeds.slice(qSize, qSize * 2).reverse(), ...seeds.slice(qSize * 3), ...seeds.slice(qSize * 2, qSize * 3).reverse()];
    let poolIdx = 0;
...
function setupTournamentStructure(list) {
    tournamentRounds = [];
    let r1 = [];
    for(let i=0; i<list.length; i+=2) r1.push({p1: list[i], p2: list[i+1], winner: null, s1: "", s2: ""});
    tournamentRounds.push(r1);
    let count = r1.length;
    while(count > 1) {
        count /= 2;
        let nr = [];
        for(let i=0; i<count; i++) nr.push({p1: "TBD", p2: "TBD", winner: null, s1: "", s2: ""});
